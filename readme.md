...
- `--detox`: Generate Detox e2e tests workflow to run on every PR.

## 🔐 GitHub token

Some generated workflows require setting up a GitHub token as a repository secret. To learn more, you can refer to 
[Using secrets in GiHub Actions](https://docs.github.com/en/actions/security-guides/using-secrets-in-github-actions). Make sure that your generated token has **read access to actions**.
Typically, workflows generated by `react-native-ci-cli` try to access the token as `${{ secrets.GH_TOKEN }}`, but you can modify the secret name if you will.
