...
Let's try to use `setup-ci` to do the heavy lifting and generate all the necessary configurations.
...
When using `setup-ci`, you can provide additional flags to modify its default behavior.
...
Some workflows generated by `setup-ci` require setting up repository secrets that can be then accessed by workflows as `${{ secrets.* }}`. Make sure to read logs printed by the script
...
This tool is supposed to be helpful to as many developers as possible - and therefore we are open to your ideas and general feedback! If you want to share your opinion about `setup-ci` or
