...
```yaml {5,6,20} title=".github/workflows/package-a-prettier.yml" showLineNumbers
name: Prettier check
...
- Line 20: the `yarn` command uses `--cwd` flag to specify in which package it should be run (for npm projects, the `npm run --prefix` option is used)
